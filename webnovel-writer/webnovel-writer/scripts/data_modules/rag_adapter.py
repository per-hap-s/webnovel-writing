...
from .api_client import RAGRequestError, get_client
...
class RAGAdapter:
    ...
    async def store_chunks(self, chunks: List[Dict[str, Any]]) -> int:
        ...
        embeddings = await self.api_client.embed_batch(contents, skip_failures=False)
        if not embeddings:
            return 0
        ...
        with self._get_conn() as conn:
            ...
            for chunk, embedding in zip(chunks, embeddings):
                ...
                try:
                    ...
                    errors.append(f"BM25 index failed for {chunk_id}: {exc}")
            ...
            try:
                ...
                errors.append(f"SQLite commit failed: {exc}")
    ...
    async def hybrid_search(
        ...
        for item in rerank_results:
            ...
            if idx < len(candidates):
                ...
                result.score = float(item.get("relevance_score", 0))
    ...
    if args.command == "stats":
        ...
        for s in scenes:
            if not isinstance(s, dict):
                continue
            scene_index = int(s.get("index", 0) or 0)
            chunk_id = f"ch{args.chapter:04d}_s{scene_index}"
            chunks.append(
                {
                    "chapter": args.chapter,
                    "scene_index": scene_index,
                    ...
                    "parent_chunk_id": parent_chunk_id,
                    "chunk_id": chunk_id,
                    "source_file": f"正文/第{args.chapter:04d}章.md#scene_{scene_index}",
                }
            )
        ...
        try:
            ...
            stage = exc.details.get("stage") or "rag"
            emit_error(exc.code, f"{stage} failed: {exc.message}", chapter=args.chapter)
            return 1
        ...
        try:
            ...
            stage = exc.details.get("stage") or "rag"
            emit_error(exc.code, f"{stage} failed: {exc.message}")
            return 1
