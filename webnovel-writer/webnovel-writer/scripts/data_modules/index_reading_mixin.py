...
class IndexReadingMixin:
    def save_chapter_reading_power(self, meta: "ChapterReadingPowerMeta"):
        ...
        将章节的钩子类型、爽点模式、微兑现等追读力相关数据写入数据库。
        如果章节记录已存在则更新，否则插入新记录。
        
        参数:
            meta: 章节追读力元数据对象，包含钩子类型、爽点模式、微兑现、违规、债务余额等信息。
        """
    ...
    def get_chapter_reading_power(self, chapter: int) -> Optional[Dict]:
        ...
        查询指定章节的追读力相关数据，包括钩子类型、爽点模式、微兑现等。
        
        参数:
            chapter: 章节号。
            
        返回:
            Optional[Dict]: 追读力元数据字典，如果章节不存在则返回 None。
        """
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute(
    ...
    def get_pattern_usage_stats(self, last_n_chapters: int = 20) -> Dict[str, int]:
        ...
        参数:
            last_n_chapters: 统计的章节数量，默认为 20。
            
        返回:
            Dict[str, int]: 爽点模式使用统计，键为模式名称，值为使用次数。
        """
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute(
    ...
    def get_hook_type_stats(self, last_n_chapters: int = 20) -> Dict[str, int]:
        ...
        统计最近章节中各种章末钩子类型的使用频次。
        
        参数:
            last_n_chapters: 统计的章节数量，默认为 20。
            
        返回:
            Dict[str, int]: 钩子类型使用统计，键为钩子类型，值为使用次数。
        """
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute(
    ...
    def save_review_metrics(self, metrics: "ReviewMetrics") -> None:
        ...
        将审查结果（包括整体评分、维度评分、严重程度统计等）写入数据库。
        如果相同章节范围的记录已存在则更新。
        
        参数:
            metrics: 审查指标对象，包含章节范围、评分、问题列表等信息。
        """
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute(
    ...
    def get_review_trend_stats(self, last_n: int = 5) -> Dict[str, Any]:
        ...
        计算最近 N 次审查的平均分数、维度平均分和严重程度总计。
        
        参数:
            last_n: 统计的审查次数，默认为 5。
            
        返回:
            Dict[str, Any]: 趋势统计结果，包含以下字段：
                - count: 审查次数
                - overall_avg: 整体平均分
                - dimension_avg: 各维度平均分
                - severity_totals: 严重程度总计
                - recent_ranges: 最近的章节范围列表
        """
    ...
    def save_writing_checklist_score(self, meta: "WritingChecklistScoreMeta") -> None:
        ...
        参数:
            meta: 写作清单评分对象，包含章节号、模板、完成项、评分等信息。
        """
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute(
    ...
    def get_writing_checklist_score(self, chapter: int) -> Optional[Dict[str, Any]]:
        ...
        返回:
            Optional[Dict[str, Any]]: 写作清单评分数据，如果不存在则返回 None。
        """
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute(
    ...
    def get_writing_checklist_score_trend(self, last_n: int = 10) -> Dict[str, Any]:
        ...
        计算最近 N 章的平均评分、完成率等趋势数据。
        
        参数:
            last_n: 统计的章节数量，默认为 10。
            
        返回:
            Dict[str, Any]: 趋势统计结果，包含以下字段：
                - count: 章节数量
                - score_avg: 平均评分
                - completion_avg: 平均完成率
                - required_completion_avg: 必选项平均完成率
                - recent: 最近章节的评分列表
        """
