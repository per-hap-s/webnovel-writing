...
class IndexEntityMixin:
    ...
    def get_entity(self, entity_id: str) -> Optional[Dict]:
        ...
        根据实体 ID 查询实体的所有属性，包括名称、类型、层级、当前状态等。
        
        参数:
            entity_id: 实体唯一标识符。
            
        返回:
            Optional[Dict]: 实体信息字典，如果实体不存在则返回 None。
        """
        with self._get_conn() as conn:
            cursor = conn.cursor()
    ...
    def get_entities_by_type(
        ...
        查询指定类型的所有实体，默认排除已归档的实体。
        
        参数:
            entity_type: 实体类型（角色/地点/物品/势力/招式）。
            include_archived: 是否包含已归档的实体，默认为 False。
            
        返回:
            List[Dict]: 实体列表，按最后出场章节降序排列。
        """
        with self._get_conn() as conn:
            cursor = conn.cursor()
            if include_archived:
    ...
    def get_core_entities(self) -> List[Dict]:
        ...
        查询层级为核心或重要，或者标记为主角的所有实体。
        主要用于 Context Agent 全量加载关键实体信息。
        
        返回:
            List[Dict]: 核心实体列表，主角排在最前，然后按层级和最后出场章节排序。
        """
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute("""
    ...
    def get_protagonist(self) -> Optional[Dict]:
        ...
        查询标记为 is_protagonist 的实体。每个项目应该只有一个主角。
        
        返回:
            Optional[Dict]: 主角实体信息，如果未设置主角则返回 None。
        """
        with self._get_conn() as conn:
            cursor = conn.cursor()
    ...
    def archive_entity(self, entity_id: str) -> bool:
        ...
        将实体标记为已归档状态，而不是物理删除。
        归档后的实体在常规查询中会被过滤，但数据仍保留在数据库中。
        
        参数:
            entity_id: 实体唯一标识符。
            
        返回:
            bool: 归档成功返回 True，实体不存在返回 False。
        """
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute(
    ...
    def remove_alias(self, alias: str, entity_id: str) -> bool:
        ...
        返回:
            bool: 移除成功返回 True，记录不存在返回 False。
        """
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute(
    ...
    def get_entity_state_changes(self, entity_id: str, limit: int = 20) -> List[Dict]:
        ...
        返回:
            List[Dict]: 状态变化记录列表，包含字段名、旧值、新值、原因、章节等信息。
        """
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute(
    ...
    def get_chapter_state_changes(self, chapter: int) -> List[Dict]:
        ...
        查询指定章节中发生的所有实体状态变化记录。
        
        参数:
            chapter: 章节号。
            
        返回:
            List[Dict]: 状态变化记录列表，按记录 ID 排序。
        """
        with self._get_conn() as conn:
            cursor = conn.cursor()
            cursor.execute(
    ...
    def get_relationship_events(
        ...
        查询与指定实体相关的所有关系事件记录，支持章节范围过滤。
        
        参数:
            entity_id: 实体唯一标识符。
            direction: 查询方向，可选值为 "from"（作为起点）、"to"（作为终点）或 "both"（双向）。
            from_chapter: 起始章节号，可选。
            to_chapter: 结束章节号，可选。
            limit: 返回数量限制，默认为 100。
            
        返回:
            List[Dict[str, Any]]: 关系事件列表，按章节号降序排列。
        """
    ...
    def get_relationship_timeline(
        ...
        返回:
            List[Dict[str, Any]]: 关系事件列表，按章节号升序排列。
        """
        clauses = [
    ...
    def build_relationship_subgraph(
        ...
        以指定实体为中心，按广度优先搜索扩展相关实体和关系，构建关系子图。
        用于可视化展示和分析实体间的关系网络。
        
        参数:
            center_entity: 中心实体的 ID。
            depth: 扩展深度，默认为 2。
            chapter: 章节截面，只返回该章节之前的关系，可选。
            top_edges: 最大返回边数，默认为 50。
            relation_types: 关系类型过滤列表，可选。
            
        返回:
            Dict[str, Any]: 关系子图，包含 center、depth、chapter、nodes、edges 等字段。
        """
    ...
    def render_relationship_subgraph_mermaid(self, graph: Dict[str, Any]) -> str:
        ...
        将关系子图数据转换为 Mermaid 图表语法，便于在 Markdown 中渲染。
        敌对关系使用虚线，友好关系使用实线。
        
        参数:
            graph: 关系子图数据，由 build_relationship_subgraph 方法返回。
            
        返回:
            str: Mermaid 图表代码块字符串。
        """
    ...
    def update_entity_field(self, entity_id: str, field: str, value: Any) -> bool:
        ...
        便捷方法，用于更新实体 current_json 中的单个字段。
