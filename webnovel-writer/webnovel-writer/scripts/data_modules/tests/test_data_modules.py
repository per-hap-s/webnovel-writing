...
class TestSQLiteConcurrency:
    ...
    def test_concurrent_reads(self, temp_project):
        """测试多线程并发读取操作
        
        验证多个线程同时读取数据库时不会发生冲突或错误。
        """
        import threading
        import queue
        ...
        def read_chapter(chapter_num: int):
            try:
                result = manager.get_chapter(chapter_num)
                results.put((chapter_num, result is not None))
            except Exception as e:
                errors.put((chapter_num, str(e)))
            finally:
                manager.close()

        threads = []
        for i in range(5):
            t = threading.Thread(target=read_chapter, args=(i + 1,))
            threads.append(t)
            t.start()
        ...
        assert errors.empty(), f"并发读取发生错误: {list(errors.queue)}"
        assert results.qsize() == 5
        for _ in range(5):
            chapter_num, success = results.get()
            assert success, f"章节 {chapter_num} 读取失败"
        
        manager.close()

    def test_concurrent_writes(self, temp_project):
        """测试多线程并发写入操作
        
        验证多个线程同时写入数据库时不会发生数据损坏或死锁。
        """
        import threading
        import queue
        ...
        def write_entity(entity_id: str):
            try:
                entity = EntityMeta(
                    id=entity_id,
                    type="角色",
                    canonical_name=f"角色_{entity_id}",
                    current={},
                    first_appearance=1,
                    last_appearance=1,
                )
                success = manager.upsert_entity(entity)
                results.put((entity_id, success))
            except Exception as e:
                errors.put((entity_id, str(e)))
            finally:
                manager.close()

        threads = []
        for i in range(10):
            t = threading.Thread(target=write_entity, args=(f"entity_{i}",))
            threads.append(t)
            t.start()
        ...
        assert errors.empty(), f"并发写入发生错误: {list(errors.queue)}"
        assert results.qsize() == 10

        for i in range(10):
            entity = manager.get_entity(f"entity_{i}")
            assert entity is not None, f"实体 entity_{i} 未找到"
        
        manager.close()

    def test_concurrent_read_write(self, temp_project):
        """测试多线程并发读写混合操作
        
        验证同时进行读写操作时数据库的一致性和稳定性。
        """
        import threading
        import queue
        import random
        ...
        def read_operation(op_id: int):
            try:
                entity_id = f"existing_{op_id % 3}"
                result = manager.get_entity(entity_id)
                results.put(("read", op_id, result is not None))
            except Exception as e:
                errors.put(("read", op_id, str(e)))
            finally:
                manager.close()
        ...
        def write_operation(op_id: int):
            try:
                entity = EntityMeta(
                    id=f"concurrent_{op_id}",
                    type="角色",
                    canonical_name=f"并发角色_{op_id}",
                    current={},
                    first_appearance=1,
                    last_appearance=1,
                )
                success = manager.upsert_entity(entity)
                results.put(("write", op_id, success))
            except Exception as e:
                errors.put(("write", op_id, str(e)))
            finally:
                manager.close()
        ...
        assert errors.empty(), f"并发读写发生错误: {list(errors.queue)}"
        assert results.qsize() == 20
        
        manager.close()

    def test_wal_mode_enabled(self, temp_project):
        """验证 WAL 模式已正确启用
        ...
        assert journal_mode is not None, "无法获取 journal_mode"
        assert journal_mode.lower() == "wal", f"WAL 模式未启用，当前模式: {journal_mode}"
        
        manager.close()

    def test_busy_timeout_configured(self, temp_project):
        """验证 busy_timeout 已正确配置
        ...
        assert busy_timeout is not None, "无法获取 busy_timeout"
        assert busy_timeout >= 5000, f"busy_timeout 应 >= 5000ms，当前: {busy_timeout}ms"
        
        manager.close()


    def test_journal_mode_falls_back_to_delete_when_wal_fails(self, monkeypatch):
        calls = []
        ...
        monkeypatch.setattr(index_manager_module.sqlite3, "connect", fake_connect)

        manager = object.__new__(IndexManager)
        manager.config = SimpleNamespace(index_db=Path("dummy-index.db"))
