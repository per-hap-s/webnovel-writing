...
def positive_int(value: str) -> int:
    ...
    Raises:
        argparse.ArgumentTypeError: 当值不是正整数时抛出
    """
    try:
        ivalue = int(value)
    except ValueError:
    ...
    if ivalue <= 0:
        raise argparse.ArgumentTypeError(f"'{value}' 必须是正整数（大于 0）")
...
def non_negative_int(value: str) -> int:
    ...
    Raises:
        argparse.ArgumentTypeError: 当值不是非负整数时抛出
    """
    try:
        ivalue = int(value)
    except ValueError:
    ...
    if ivalue < 0:
        raise argparse.ArgumentTypeError(f"'{value}' 必须是非负整数（大于等于 0）")
...
示例：
  ...
  python update_state.py \
    ...
    parser.add_argument(
        ...
        help='更新金手指（名称 等级[正整数] 冷却天数[非负整数]）'
    )
    ...
    try:
        if args.protagonist_power:
            ...
            try:
                layer_int = positive_int(layer)
            except argparse.ArgumentTypeError as e:
                print(f"❌ 参数错误: --protagonist-power 的 LAYER {e}")
                sys.exit(1)
            updater.update_protagonist_power(realm, layer_int, bottleneck)
        ...
        if args.golden_finger:
            ...
            try:
                level_int = positive_int(level)
            except argparse.ArgumentTypeError as e:
                print(f"❌ 参数错误: --golden-finger 的 LEVEL {e}")
                sys.exit(1)
            try:
                cooldown_int = non_negative_int(cooldown)
            except argparse.ArgumentTypeError as e:
                print(f"❌ 参数错误: --golden-finger 的 COOLDOWN {e}")
                sys.exit(1)
            updater.update_golden_finger(name, level_int, cooldown_int)
