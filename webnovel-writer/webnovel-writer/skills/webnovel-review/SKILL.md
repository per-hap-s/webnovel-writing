...
- Webnovel CLI 的“工作区根目录”不一定等于“书项目根目录”。常见结构：工作区为 `D:\wk\xiaoshuo`，书项目为 `D:\wk\xiaoshuo\凡人资本论`。
...
if [ -z "${WEBNOVEL_APP_ROOT}" ] || [ ! -d "${WEBNOVEL_APP_ROOT}/skills/webnovel-review" ]; then
  echo "ERROR: 未设置 WEBNOVEL_APP_ROOT 或缺少目录: ${WEBNOVEL_APP_ROOT}/skills/webnovel-review" >&2
  exit 1
fi
export SKILL_ROOT="${WEBNOVEL_APP_ROOT}/skills/webnovel-review"
...
## Step 3: 并行调用检查员（subtask）

**调用约束**:
- 必须通过 `subtask` 工具调用审查 subagent，禁止主流程直接内联审查结论。
...
如发现 critical 问题（`severity_counts.critical > 0` 或 `critical_issues` 非空），**必须使用 question-step** 询问用户：
- A) 立即修复（推荐）
- B) 仅保存报告，稍后处理
