...
- **禁止**在插件目录 `${WEBNOVEL_APP_ROOT}/` 下读取或写入项目文件

环境设置（bash 命令执行前）：
```bash
export WORKSPACE_ROOT="${WEBNOVEL_WORKSPACE_ROOT:-$PWD}"

if [ -z "${WEBNOVEL_APP_ROOT}" ] || [ ! -d "${WEBNOVEL_APP_ROOT}/skills/webnovel-query" ]; then
  echo "ERROR: 未设置 WEBNOVEL_APP_ROOT 或缺少目录: ${WEBNOVEL_APP_ROOT}/skills/webnovel-query" >&2
  exit 1
fi
export SKILL_ROOT="${WEBNOVEL_APP_ROOT}/skills/webnovel-query"
