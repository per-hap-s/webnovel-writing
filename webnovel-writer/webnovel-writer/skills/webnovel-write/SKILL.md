...
- `subtask`：调用 `context-agent`、审查 subagent、`data-agent` 并行执行。
...
- 校验核心输入：`大纲/总纲.md`、`${WEBNOVEL_APP_ROOT}/scripts/extract_chapter_context.py` 存在。
- 规范化变量：
  - `WORKSPACE_ROOT`：Webnovel CLI 打开的工作区根目录（可能是书项目的父目录，例如 `D:\wk\xiaoshuo`）
  - `PROJECT_ROOT`：真实书项目根目录（必须包含 `.webnovel/state.json`，例如 `D:\wk\xiaoshuo\凡人资本论`）
  - `SKILL_ROOT`：skill 所在目录（固定 `${WEBNOVEL_APP_ROOT}/skills/webnovel-write`）
  - `SCRIPTS_DIR`：脚本目录（固定 `${WEBNOVEL_APP_ROOT}/scripts`）
  - `chapter_num`：当前章号（整数）
  - `chapter_padded`：四位章号（如 `0007`）

环境设置（bash 命令执行前）：
```bash
# WORKSPACE_ROOT：Webnovel CLI 的工作区根（通常等于 $WEBNOVEL_WORKSPACE_ROOT）
export WORKSPACE_ROOT="${WEBNOVEL_WORKSPACE_ROOT:-$PWD}"

if [ -z "${WEBNOVEL_APP_ROOT}" ] || [ ! -d "${WEBNOVEL_APP_ROOT}/skills/webnovel-write" ]; then
  echo "ERROR: 未设置 WEBNOVEL_APP_ROOT 或缺少目录: ${WEBNOVEL_APP_ROOT}/skills/webnovel-write" >&2
  exit 1
fi
export SKILL_ROOT="${WEBNOVEL_APP_ROOT}/skills/webnovel-write"

if [ -z "${WEBNOVEL_APP_ROOT}" ] || [ ! -d "${WEBNOVEL_APP_ROOT}/scripts" ]; then
  echo "ERROR: 未设置 WEBNOVEL_APP_ROOT 或缺少目录: ${WEBNOVEL_APP_ROOT}/scripts" >&2
  exit 1
fi
...
使用 subtask 调用 `context-agent`，参数：
- `chapter`
- `project_root`
...
### Step 3：审查（auto 路由，必须由 subtask 子代理执行）
...
调用约束：
- 必须用 `subtask` 调用审查 subagent，禁止主流程伪造审查结论。
