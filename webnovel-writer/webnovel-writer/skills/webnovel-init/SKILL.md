...
3. 允许调用 `Read/Grep/Bash/subtask/question-step/WebSearch/WebFetch` 辅助收集。
...
路径约定：
- `references/...` 相对当前 skill 目录（`${WEBNOVEL_APP_ROOT}/skills/webnovel-init/references/...`）。
- `templates/...` 相对插件根目录（`${WEBNOVEL_APP_ROOT}/templates/...`）。
...
- `subtask`：拆分并行子任务（如题材映射、约束包候选生成、文件验证）。
- `question-step`：用于关键分歧裁决、候选方案选择、最终确认。
...
- 解析脚本目录并确认入口存在（仅支持插件目录）：
  - 固定路径：`${WEBNOVEL_APP_ROOT}/scripts`
...
- 禁止在插件目录下生成项目文件（`${WEBNOVEL_APP_ROOT}`）。
